...
def read_books(skip: int = 0, limit: int = 10, db: Session = Depends(dependencies.get_db)):
    books = crud.get_books(db, skip=skip, limit=limit)
    return books

@router.post("/borrow/", response_model=schemas.BorrowedBook)
def borrow_book(borrow: schemas.BorrowedBookCreate, db: Session = Depends(dependencies.get_db), current_user: models.User = Depends(dependencies.get_current_user)):
...
def return_book(borrow_id: int, db: Session = Depends(dependencies.get_db), current_user: models.User = Depends(dependencies.get_current_user)):
    borrow_record = db.query(models.BorrowedBook).filter(models.BorrowedBook.id == borrow_id, models.BorrowedBook.user_id == current_user.id).first()
